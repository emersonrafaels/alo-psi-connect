// Bundled MCP Edge Function. Keep synchronized with src/lib/mcp/tools.
...
var search_professionals_default = defineTool({
  ...
  handler: async ({ query, limit }) => {
    ...
    let q = supabase.from("profissionais").select("id, profile_id, display_name, profissao, resumo, foto_perfil_url").eq("ativo", true).limit(cap);
    if (query && query.trim()) {
...
var list_upcoming_group_sessions_default = defineTool2({
  ...
  handler: async ({ limit }) => {
    ...
    const { data, error } = await supabase.from("group_sessions").select("id, title, description, session_date, start_time, duration_minutes, max_participants, current_registrations, professional:profissionais!group_sessions_professional_id_fkey(display_name)").gte("session_date", today).in("status", ["scheduled", "live"]).order("session_date", { ascending: true }).order("start_time", { ascending: true }).limit(cap);
    if (error) {
      return {
        content: [{ type: "text", text: `Error: ${error.message}` }],
        isError: true
