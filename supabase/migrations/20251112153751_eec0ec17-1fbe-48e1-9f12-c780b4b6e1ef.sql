-- Create Medcos-specific minimal professional availability function
-- Returns active professionals from Medcos tenant only, with normalized services and academic background
-- Optimized for LLM consumption with ~80-85% token reduction vs full RPC
...
COMMENT ON FUNCTION public.get_professionals_with_filtered_availability_minimal_medcos IS 
'Medcos-specific minimal professional availability function. Returns active professionals from Medcos tenant only, with normalized services and academic background optimized for LLM consumption. Reduces token usage by ~80-85% compared to full RPC.';