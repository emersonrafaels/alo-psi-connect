...
BEGIN
  ...
  IF v_user_count < 5 THEN
    RETURN jsonb_build_object('available', false, 'reason', 'insufficient_users', 'min_required', 5, 'current', v_user_count);
  END IF;
  ...
  SELECT jsonb_build_object(
    ...
    'total_students', (SELECT COUNT(*) FROM patient_institutions WHERE institution_id = p_institution_id),
    'total_entries', COUNT(*),
