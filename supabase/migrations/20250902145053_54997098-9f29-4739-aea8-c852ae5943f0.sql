...
USING (auth.uid()::text = (SELECT user_id::text FROM public.profiles WHERE id = profile_id));

CREATE POLICY "Professionals can insert their own profile" 
ON public.profissionais FOR INSERT 
WITH CHECK (auth.uid()::text = (SELECT user_id::text FROM public.profiles WHERE id = profile_id));
...
USING (user_id IN (SELECT user_id FROM public.profissionais WHERE user_id = profissionais_sessoes.user_id));