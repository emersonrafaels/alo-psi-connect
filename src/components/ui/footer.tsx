...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    const success = await subscribe({ email, nome: nome || undefined });
    if (success) {
      setEmail("");
      setNome("");
    }
  ...
  const links = {
    useful: [
      ...
      { name: "Nossos Profissionais", href: "/professionals", icon: Heart },
      { name: "Agendar Consulta", href: "/appointment", icon: Calendar },
      { name: "Blog", href: "/blog", icon: FileText },
      { name: "Trabalhe Conosco", href: "/work-with-us", icon: MessageCircle }
    ],
    navigation: [
      { name: "Home", href: "/" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-3">
              <Input 
                ...
                className="bg-transparent border-primary-foreground/20 text-primary-foreground placeholder:text-primary-foreground/60" 
              />
          ...
          <div className="lg:col-span-1">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              ...
              <a 
                href="https://www.instagram.com/medcos_br/" 
                target="_blank" 
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {Array.from({ length: 6 }).map((_, index) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="aspect-square bg-teal rounded-md hover:opacity-80 transition-opacity cursor-pointer"
                />
              ))}
            </div>
